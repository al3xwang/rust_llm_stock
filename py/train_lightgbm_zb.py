#!/usr/bin/env python3
"""
LightGBM Training Script for ZB Stock Data

This script trains a LightGBM model on ZB (Shanghai & Shenzhen) stock market data,
evaluates on validation set, and tests on test_data.csv.
"""
...
print("="*70)
print("🚀 LIGHTGBM ZB STOCK MARKET MODEL TRAINING")
print("="*70)
...
# If test_data.csv is empty, use validation data as test
if df_test_loaded.shape[0] == 0:
    print(f"ℹ️  test_data.csv is empty ({df_test_loaded.shape[0]} rows)")
    print(f"   Using validation set (zb_val.csv) as test data instead")
    df_test = df_val.copy()
    test_source = "validation set (zb_val.csv)"
else:
    df_test = df_test_loaded
    test_source = "test_data.csv"

print(f"✅ Test data loaded: {df_test.shape} from {test_source}")
...
if has_test_target:
    ...
    print(f"   ℹ️  No target column in test data")
...
# Save max probability for each class
pred_df['predicted_proba'] = y_pred_test_proba.max(axis=1)
...
if has_test_target:
    ...
    test_auc = roc_auc_score(y_test_lgb, y_pred_test_proba)
    ...
    plt.tight_layout()
    plot_path = Path("./artifacts/test_evaluation_lgb.png")
    ...
    ax.hist(y_pred_test_proba.max(axis=1), bins=50, color='steelblue', alpha=0.7, edgecolor='black')
    ...
    plot_path = Path("./artifacts/test_predictions_distribution_lgb.png")
    plt.savefig(plot_path, dpi=100, bbox_inches='tight')
...
print(f"   - Evaluation plot: ./artifacts/test_evaluation_lgb.png")
