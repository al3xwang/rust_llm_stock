...
use rust_llm_stock::{
    ...
    ts::{ http::{ Responsex, TradeRecord }, model::{ StockBasic, StockKey } },
};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let stock_list: Vec<String> = sqlx::query_scalar("SELECT ts_code FROM stock_basic WHERE list_status = 'L'")
        .fetch_all(&dbpool)
        .await?;

    // Default starting date for monthly data (month of 2020-12-14)
...
pub async fn pull_insert(
    ...
    println!("start pulling monthly data for {} from {} ...", ts_code, &date);
    ...
    let response = client
        ...
        .body(create_req("monthly".to_string(), params))
        .send().await?;
    ...
    for item in &repx.data.items {
        ...
        let query_result = sqlx::query(
            "INSERT INTO monthly (ts_code,trade_date,open,high,low,close,pre_close,change,pct_chg,vol,amount)
    ...
    tx.commit().await?;
    println!("Completed for {}: inserted={}, skipped={}", ts_code, inserted_count, skipped_count);
