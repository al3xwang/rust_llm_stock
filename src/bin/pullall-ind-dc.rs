// Incremental ingestion for industry money flow data from DC sources (moneyflow_ind_dc)
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Get the latest date from moneyflow_ind_dc table, or use default start date
    ...
    let start_date = if let Some(user_date) = user_start_date {
        ...
        println!("No data in DB, starting from default: 20240101");
        "20240101".to_string()
    };
    ...
    println!(
        "Pulling industry money flow (DC) data from {} to {}",
        start_date, end_date
    );
    ...
    while current <= today {
        ...
        println!(
            "[{}/{}] Pulling industry moneyflow (DC) from {} to {}",
            processed_months,
            total_months.max(1),
            start_date_month,
            end_date_str
        ...
        match ingest_industry_moneyflow_dc(&pool, &start_date_month, &end_date_str).await {
            Ok(count) => {
                if count > 0 {
...
async fn ingest_industry_moneyflow_dc(
    pool: &Pool<Postgres>,
    start_date: &str,
    end_date: &str,
    ...
    let request_body = create_req("moneyflow_ind_dc".to_string(), params);
        ...
        if let (Some(i_ts), Some(i_date), Some(i_ind), Some(i_buy), Some(i_sell), Some(i_net)) = (
            ...
            if item.len() > i_net {
                ...
                match insert_result {
                    ...
                    Err(e) => {
                        eprintln!(
                            "    Error inserting industry moneyflow (DC) for {} on {}: {}",
